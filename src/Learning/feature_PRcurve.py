...
if __name__ == '__main__':
    ...
    y_real2 = []
    y_proba2 = []
    X2, y2 = load_data(classify_data,"../../data/feature_vec/f2.npy")
    ...
    X3, y3 = load_data(classify_data,"../../data/feature_vec/f3.npy")
    y_real3 = []
    y_proba3 = []
    for i, (train_index, test_index) in enumerate(k_fold.split(X3)):
        ...
        lab = 'Fold %d AUC=%.4f' % (i+1, auc(recall, precision))
        # axes.plot(recall, precision, label=lab)
        y_real3.append(ytest)
        y_proba3.append(pred_proba[:,1])

    y_real3 = np.concatenate(y_real3)
    y_proba3 = np.concatenate(y_proba3)
    precision, recall, _ = precision_recall_curve(y_real, y_proba3)
